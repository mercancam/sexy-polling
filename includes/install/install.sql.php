<?php 
global $wpdb;
global $wpsxp_db_version;

add_option("wpsxp_db_version", $wpsxp_db_version);

require_once(ABSPATH . '/wp-admin/includes/upgrade.php');

$sql =
"
CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."wpsxp_sexy_polls` (
  `id` int(10) unsigned NULL AUTO_INCREMENT,
  `id_user` int(10) unsigned NULL,
  `id_category` mediumint(8) unsigned NULL,
  ...
  `multiple_answers` enum('0','1') NULL DEFAULT '0',
  `voting_permission` int(10) unsigned NULL DEFAULT '1',
  `number_answers` mediumint(8) unsigned NULL,
  `voting_period` float unsigned NULL,
  `answerpermission` int(10) unsigned NULL DEFAULT '1',
  `autopublish` enum('0','1') NULL DEFAULT '1',
  `baranimationtype` text NULL,
  `coloranimationtype` text NULL,
  `reorderinganimationtype` text NULL,
  `dateformat` text NULL,
  `autoopentimeline` enum('0','1') NULL DEFAULT '1',
  `autoanimate` enum('0','1') NULL DEFAULT '0',
  `showresultbutton` enum('0','1') NULL DEFAULT '1',
  PRIMARY KEY (`id`),
  KEY `id_user` (`id_user`),
...
$sql =
"
INSERT IGNORE INTO `".$wpdb->prefix."wpsxp_sexy_polls` (`id`, `id_user`, `id_category`, `width`, `id_template`, `name`, `alias`, `question`, `created`, `date_start`, `date_end`, `dateformat`, `published`, `ordering`, `multiple_answers`,`access`,`number_answers`,`voting_period`,`baranimationtype`,`coloranimationtype`,`reorderinganimationtype`) VALUES
(1, 0, 1, '98%', 1, 'Sexy Poll Example', '', 'Do You like Sexy Polling by 2GLux?', '2012-04-19 19:13:34', '0000-00-00', '0000-00-00', '1', '1', '0', '1', '1','3','24','linear','linear','linear');
";
$wpdb->query($sql);

$sql =
"
CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."wpsxp_sexy_answers` (
  `id` int(10) unsigned NULL AUTO_INCREMENT,
  `id_poll` int(10) unsigned NULL,
  `id_user` int(10) unsigned NULL,
  `name` text NULL,
  `published` tinyint(1) NULL,
  `publish_up` datetime NULL,
  `publish_down` datetime NULL,
  `created` datetime NULL,
  `ordering` mediumint(8) unsigned NULL,
  PRIMARY KEY (`id`),
  KEY `id_poll` (`id_poll`),
  KEY `id_user` (`id_user`),
  KEY `order` (`ordering`)
...
$sql =
"
INSERT IGNORE INTO `".$wpdb->prefix."wpsxp_sexy_answers` (`id`, `id_poll`, `id_user`, `name`, `published`, `created`, `ordering`) VALUES
...
$sql =
"
CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."wpsxp_sexy_votes` (
  `id_answer` int(10) unsigned NULL,
  `ip` text NULL,
  `date` datetime NULL,
  `country` text NULL,
  `city` text NULL,
...
$sql =
"
INSERT IGNORE INTO `".$wpdb->prefix."wpsxp_sexy_votes` (`id_answer`, `ip`, `date`,`country`,`city`,`region`,`countrycode`) VALUES
...
$sql =
"
CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."wpsxp_sexy_categories` (
...
$sql =
"
INSERT IGNORE INTO `".$wpdb->prefix."wpsxp_sexy_categories` (`id`, `name`, `published`, `ordering`) VALUES
(1, 'Uncategorized', 1, 0);
";
$wpdb->query($sql);
...
$sql =
"
CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."wpsxp_sexy_templates` (
  `id` mediumint(8) unsigned NULL AUTO_INCREMENT,
  `name` text NULL,
  `styles` text NULL,
   `ordering` int(11) NULL,
...
$sql =
"
INSERT IGNORE INTO `".$wpdb->prefix."wpsxp_sexy_templates` (`id`, `name`, `styles`, `published`) VALUES

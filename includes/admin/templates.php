...
if($task == 'unpublish') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "UPDATE ".$wpdb->prefix."wpsxp_sexy_templates SET `published` = '0' WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
if($task == 'publish') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "UPDATE ".$wpdb->prefix."wpsxp_sexy_templates SET `published` = '1' WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
if($task == 'delete') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "DELETE FROM ".$wpdb->prefix."wpsxp_sexy_templates WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
$sql = 
		"
			SELECT 
				...
				sp.name,
				sp.published
			FROM ".$wpdb->prefix."wpsxp_sexy_templates sp
			WHERE 1 
		";
if($filter_state == 1)
...
?>
<form action="admin.php?page=sexypollingtemplates" method="post" id="wpsxp_form">
<div style="overflow: hidden;margin: 0 0 10px 0;">
	<div style="float: left;">
		...
		<a href="admin.php?page=sexypollingtemplates"  class="button">Reset</a>
	</div>
	<div style="float:right;">
		<a href="admin.php?page=sexypollingtemplates&act=new" id="wpsxp_add" class="button-primary">New</a>
...
<tbody id="wpsxp_sortable" table_name="<?php echo $wpdb->prefix;?>wpsxp_sexy_templates" reorder_type="reorder">
<?php        
			$k = 0;
...
?>
				<tr class="row<?php echo $k; ?> ui-state-default" id="option_li_<?php echo $row->id; ?>">
					...
					<td valign="middle" align="left" style="vertical-align: middle;padding-left: 22px;">
						<a href="admin.php?page=sexypollingtemplates&act=edit&id=<?php echo $row->id;?>"><?php echo $row->name; ?></a>
					</td>
					<td valign="middle" align="center" style="vertical-align: middle;">
						<?php echo $row->id; ?>
					</td>

...
$filter_poll = isset($_REQUEST['filter_poll']) ? (int) $_REQUEST['filter_poll'] : 0;
...
if($task == 'unpublish') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "UPDATE ".$wpdb->prefix."wpsxp_sexy_answers SET `published` = '0' WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
if($task == 'publish') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "UPDATE ".$wpdb->prefix."wpsxp_sexy_answers SET `published` = '1' WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
if($task == 'delete') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "DELETE FROM ".$wpdb->prefix."wpsxp_sexy_answers WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
$sql = 
		"
			SELECT 
				...
				COUNT(sv.id_answer) as count_votes
			FROM ".$wpdb->prefix."wpsxp_sexy_answers  sp
			LEFt JOIN ".$wpdb->prefix."wpsxp_sexy_polls AS sf ON sf.id=sp.id_poll
			LEFt JOIN ".$wpdb->prefix."wpsxp_sexy_votes AS sv ON sv.id_answer=sp.id
			WHERE 1 
		";
...
if($filter_poll != 0) {
	$sql .= " AND sp.id_poll = '".$filter_poll."'";
}

$sql .= " GROUP BY sp.id ORDER BY `poll_id`,`ordering`,`id` ASC";
...
?>
<form action="admin.php?page=sexyanswers" method="post" id="wpsxp_form">
<div style="overflow: hidden;margin: 0 0 10px 0;">
	<div style="float: left;">
		...
		<select id="wpsxp_filter_poll" class="wpsxp_select" name="filter_poll">
			<option value="0">Select Poll</option>
			<?php 
				foreach($polls_rows as $poll) {
					$selected = $filter_poll == $poll->id ? 'selected="selected"' : '';
		...
		<a href="admin.php?page=sexyanswers"  class="button">Reset</a>
	</div>
	<div style="float:right;">
		<a href="admin.php?page=sexyanswers&act=new" id="wpsxp_add" class="button-primary">New</a>
...
<table class="widefat" >
	<thead>
		<tr>
			...
			<th nowrap align="left" style="text-align: left;">Poll</th>
			<th nowrap align="center" style="width: 30px;text-align: center;">Votes</th>
			<th nowrap align="center" style="width: 30px;text-align: center;">Id</th>
		</tr>
	</thead>
<tbody id="wpsxp_sortable" table_name="<?php echo $wpdb->prefix;?>wpsxp_sexy_answers" reorder_type="reorder_list">
<?php        
			$k = 0;
			for($i=0; $i < count( $rows ); $i++) {
				$row = $rows[$i];
?>
				<tr class="row<?php echo $k; ?> ui-state-default" id="option_li_<?php echo $row->id; ?>_<?php echo $row->poll_id; ?>">
					...
					<td valign="middle" align="left" style="vertical-align: middle;padding-left: 22px;" now_w="1">
						<a href="admin.php?page=sexyanswers&act=edit&id=<?php echo $row->id;?>"><?php echo $row->name; ?></a>
					</td>
					<td valign="middle" align="left" style="vertical-align: middle;" now_w="1">
						<a href="admin.php?page=sexypolls&act=edit&id=<?php echo $row->poll_id;?>"><?php echo $row->poll; ?></a>
					</td>
					<td valign="middle" align="center" style="vertical-align: middle;width: 30px;">
